...
if len(sys.argv) < 8:
    print "USAGE: python generate_feature_vec.py <path_to_training_data> <path_to_test_files> <path_to_decision_tree_list> <path_to_year_dict> <path_to_titles> <feature_list> <version_number>"
    sys.exit()

# time python testing_feature_vec.py articles/ TEST.files decision.trees ../../fileToYear.dict titles.out '50_50 abstract common_authors papers_published_best papers_published_next_best paper_year' 1
...
titles_path = sys.argv[5].rstrip("/")
feature_list = sys.argv[6].split()
version_number = sys.argv[7]
...
f_restricted_set = open ('/Users/user/Desktop/Data/New/restricted.set', 'r')
...
for file in test_file_list:
    ...
    # candidate_contexts = getContexts(file)
    candidate_contexts = restricted_set.get (int (file), [])
    papers_published_best = getAuthorPubCount(authors[0],authors[1],int(file),1)
    papers_published_next_best = getAuthorPubCount(authors[0],authors[1],int(file),2)
    paper_year = paperyears.get (int(file), 0)
    for context in candidate_contexts:
        if totCount % 50 == 0:
            print totCount
        totCount += 1
        if totCount < lowerTrigger:
            continue
        if totCount > upperTrigger:
            continue

        feature_vec_file_path = directory_path + os.sep + 'cit/' + context + os.sep + context.split(os.sep)[-1] + ".v" + version_number + ".arff"

        if os.path.isfile (feature_vec_file_path):
            continue
        ...
        for document in decision_tree_list:
            ...
            for feature in feature_list:                        
                if feature == "papers_published_best":
                    feature_vec.append(papers_published_best)

                elif feature == "papers_published_next_best":
                    feature_vec.append(papers_published_next_best)
                    ...
                    context_tfidf_path = context + os.sep + context.split(os.sep)[-1] + "_" + feature + ".words.tfidf.gz"
